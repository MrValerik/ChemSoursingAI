...
export interface SupplierRead {
   ...
   * компанию назвала площадка и своей страницы у нас нет, её нашли на
   * чужой странице — в справочнике, обзоре рынка или статье, — либо
   * собственный сайт компании искали по названию и не нашли.
   */
  contact_barrier:
    | "obfuscated"
    | "form"
...
export interface QualifiedSupplierResult extends SupplierSearchResult {
   ...
   * площадки. Такая находка в список идёт и потому видна сразу — но с
   * пометкой: имя оттуда верное, а сказано о компании с чужих слов.
   */
  third_party?: boolean;
  red_flags: string[];
  missing_evidence: string[];
