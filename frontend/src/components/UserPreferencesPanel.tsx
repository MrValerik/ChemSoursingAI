...
export default function UserPreferencesPanel({ readOnly = false }: { readOnly?: boolean }) {
  ...
  return <section className="panel">
    ...
    <p className="note">Применяется к поискам, которые запускаете вы. После завершения
      поиска система отправит RFQ по Email всем компаниям из таблицы «Отобранные компании»
      с доступным адресом, включая неподтверждённых кандидатов. Исключённые и отклонённые
      компании пропускаются. Повторные письма не отправляются. Включение разрешает
