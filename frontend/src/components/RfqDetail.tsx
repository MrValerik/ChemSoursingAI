...
// Пояснения ко всем поддерживаемым базисам. Набор обязан покрывать
// SUPPORTED_INCOTERMS из backend/app/services/incoterms.py: незнакомый код
// молча выпадает из объяснения, и закупщик видит условия без разъяснения.
// Сохранённые запросы могут содержать и базис вне набора — на этот случай
// ниже стоит запасная строка.
...
export default function RfqDetail({
  ...
  const incotermsExplanation =
    incoterms
      .map(
        (code) =>
          INCOTERM_HELP[code.toLocaleUpperCase()] ||
          `${code.toLocaleUpperCase()}: базис из сохранённого запроса, пояснения к нему нет.`,
      )
      .join(" ") ||
      ...
      <div className="detail-layout">
        <aside className="detail-params panel">
          <h2>
            <Term
              label="Параметры"
              help="Основные условия закупочного запроса. Подсказка открывается при наведении на термин или значение."
            />
          </h2>
