        ...
        "queries": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "purpose": {
                        "type": "string",
                        "enum": ["manufacturer", "product", "documents"],
                    },
                    # registry из перечня убран: за месяц этот тип дал 18
                    # запросов, 61% пустых и ноль карточек. Литералы
                    # SearchPlanItem его ещё принимают, чтобы сохранённые
                    # планы старых прогонов читались как прежде.
                    "source_type": {
                        "type": "string",
                        "enum": ["official_site", "catalog", "web"],
                    },
                    "priority": {
                        "type": "integer",
                        "minimum": 1,
...
def _fallback_search_plan(
    ...
    for index, query in enumerate(queries):
        ...
        # Ветки под отраслевые реестры Индии убраны вместе с самими
        # запросами: 18 запросов за месяц, 61% пустых, ноль карточек.
        if any(marker in query for marker in ("CoA", "TDS", "SDS")):
            purpose = "documents"
