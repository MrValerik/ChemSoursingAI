...
class Settings(BaseSettings):
    ...
    # --- Отрисовка страниц браузером (служба renderer) ---
    # Часть сайтов присылает не страницу, а программу: замер 3 сентября 2026
    # по пяти адресам с ошибкой «на странице не найден текст» дал ответы 200
    # длиной 151-985 байт из одного упакованного скрипта. Обычный запрос
    # такую страницу прочитать не может по определению.
    #
    # Пустой адрес выключает отрисовку целиком: она не обязательна, и без
    # неё поиск работает ровно как раньше.
    page_render_url: str = Field(default="")
    page_render_timeout_s: int = Field(default=30, ge=5, le=120)
    # Ниже этого порога считаем, что страницу прочитать не удалось, и
    # пробуем открыть её браузером. 400 символов - это шапка меню без
    # содержимого; настоящая товарная страница короче почти не бывает.
    page_render_min_chars: int = Field(default=400, ge=0, le=100_000)
    ...
    # Потолок расхода токенов на один запрос поиска: вход и выход вместе,
    # по всем этапам запуска. Считать по числу вызовов недостаточно: один
    # разрешённый вызов дробится на половины и поштучные дозапросы, и
    # стоимость растёт там, где счётчик вызовов стоит на месте.
    #
    # Замеры, которые у нас есть, поэтапные, не по запуску: карбомер —
    # 20 722 входных и 4 753 выходных на семь вызовов одного этапа; аудит
    # прогона 55 — 8 658 входных. Этапов в запуске больше, и оценка с
    # аудитом идут пакетами по числу кандидатов, поэтому запуск стоит
    # заметно дороже 25 тысяч. Стоимость запуска целиком на боевых данных
    # не замерена; 100 000 — рабочая отсечка примерно вчетверо выше
    # известного этапа. Если честные прогоны начнут в неё упираться, это
    # видно по stop reason token_budget_exhausted в трассе: значение
    # поднимается настройкой, без правки кода. Ноль выключает лимит.
    search_max_tokens: int = Field(default=100_000, ge=0)
