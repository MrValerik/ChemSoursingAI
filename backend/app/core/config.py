...
class Settings(BaseSettings):
    ...
    # стоимость растёт там, где счётчик вызовов стоит на месте. Значение
    # взято с запасом над теми замерами отдельных этапов, что у нас есть
    # (карбомер: 20 722 входных и 4 753 выходных на семь вызовов; аудит
    # прогона 55: 8 658 входных), и рассчитано ловить зацикливание, а не
    # подрезать честный прогон. Стоимость запуска целиком на боевых
    # данных не замерена — значение калибруется по ней, когда появится.
    # Ноль выключает ограничение.
    search_max_tokens: int = Field(default=400_000, ge=0)
