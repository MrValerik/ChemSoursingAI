...
_DEFAULT_PROMPTS = [
    ...
    (
        ...
        "прямо сообщи об этом. confidence означает уверенность в корректности "
        "твоей классификации, а не вероятность принятия документа. Если контекст "
        "явно помечен synthetic_demo, проверь внутреннее соответствие тестового "
        "файла по тем же цитатам, CAS и партии; ожидаемую маркировку demo не считай "
        "несоответствием вещества. Пояснения пиши по-русски.",
    ),
    (
        "followup",
