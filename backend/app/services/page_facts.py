...
# Следы присутствия компании в стране, которые читаются со страницы без
# модели. Замер по 48 карточкам прогонов 280-320, где страна осталась
# ненайденной: лицензия ICP нашлась у двух, телефон с кодом страны — у
# пяти, доменная зона страны — у двенадцати.
#
# Правило намеренно однобокое: признак может подтвердить искомую страну и
# никогда не опровергает её. Чужой телефон на странице — обычное дело у
# торговой компании, которая перечисляет поставщиков, и делать из него
# вывод «компания не там» нельзя. Опровержение остаётся за моделью,
# которая читает эту цитату в контексте.
_ICP_RE = re.compile(r"[^\n]{0,60}ICP备[^\n]{0,40}")

# Код страны в международном формате. Требуется знак «+» и разделитель:
# «+8613800138000» и «+86 138-0013-8000» подходят, а «86» внутри артикула
# — нет.
_PHONE_COUNTRY_RE = {
    "cn": re.compile(r"[^\n]{0,40}\+86[\s\-()]?\d[\d\s\-()]{6,}[^\n]{0,20}"),
    "in": re.compile(r"[^\n]{0,40}\+91[\s\-()]?\d[\d\s\-()]{6,}[^\n]{0,20}"),
...
def find_country_markers(text: str, code: str) -> str | None:
    """Дословная строка страницы, говорящая о присутствии в этой стране.

    ``code`` — «cn», «in» или «ru». Возвращает цитату либо ``None``.
    """
    body = text or ""
    if code == "cn":
        # Номер лицензии ICP выдаётся только сайтам, размещённым в
        # материковом Китае, и стоит в подвале страницы.
        match = _ICP_RE.search(body)
        if match:
            quote = match.group(0).strip()
            if len(quote) >= MIN_QUOTE_CHARS:
                return quote[:_MAX_LINE_CHARS]
    pattern = _PHONE_COUNTRY_RE.get(code)
    if pattern is None:
        return None
