...
сохранённых выдач: сайт-кандидат нашёлся у всех шести, и среди них был
`importgenius.cn` — агрегатор судовых записей, у которого имя компании
стоит прямо в заголовке страницы. По заголовку он проходит проверку, по
домену — нет. Без этого правила агрегаторы попали бы в реестр как сайты
поставщиков, а письма — их владельцам.

Цена правила — пропуск сайтов под торговой маркой: `senwayer.com`
принадлежит Dingwang Technology (Wuhan), но её имени в домене нет.
Пропуск закупщик закроет руками, ложный контакт — нет.
