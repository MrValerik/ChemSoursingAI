...
- Кандидата, который запрету противоречит, верни в поле `rejected`, а не в
  `candidates`, и назови в `reason` нарушенное ограничение. Молча
  выбрасывать нельзя: закупщик должен видеть, что замена была и почему её
  сняли.
...
def suggest_analogs(
    ...
    *,
    ...
    for item in raw.get("rejected") or []:
        ...
        suggestion.warnings.append(
            f"Ограничением снят кандидат «{name}»"
    ...
    if not suggestion.candidates:
        suggestion.warnings.append(
            ...
            if not (raw.get("rejected") or [])
            else "Все найденные замены сняты вашим ограничением. Смягчите "
            "его или уточните формулировку — что именно нельзя закупать."
        )
    return suggestion
