...
# Домены, которые вычёркиваются прямо в запросе. Это подмножество
# NON_MANUFACTURER_DOMAINS: те, что чаще всего занимают места в выдаче.
# Замер по 21 436 сохранённым находкам за 27 июля - 20 августа 2026: на
# этих десяти доменах 2700 находок, 13% всей выдачи, и ни одна из них ни
# разу не дала поставщика с названной ролью. Открывать их мы и так
# отказываемся, то есть платим за них местами в выдаче впустую.
#
# Проверено на самом Serper 1 сентября 2026, три запроса с исключениями и
# без: оператор применяется, восемь исключений подряд тариф не отклоняет,
# а выдача возвращает на одну ссылку больше. Мёртвых находок стало ноль во
# всех трёх случаях. На позициях с CAS освободившиеся места достались
# страницам заводов (biofingredients, chuanghaibio, sinofoodsupply,
# lifelongchems). На торговой марке места забрал сам владелец марки своими
# же страницами - там выигрыша нет, но и потери нет: страница китайского
# завода осталась на месте.
#
# Список закрытый и короткий намеренно: длинный запрос теряет выдачу, а
# площадки с витринами (made-in-china, alibaba, lookchem, guidechem,
# chemball, echemi) вычёркивать нельзя - магазин одной компании там
# единственная первичная страница завода.
...
_EXCLUSION_SUFFIX = " " + " ".join(
    f"-site:{domain}" for domain in SEARCH_EXCLUDED_DOMAINS
)


# Назначения запросов, которые опознают вещество, а не ищут поставщика.
# Для них исключения не применяются: у торговой марки без номера каталог —
# единственный источник состава, INCI и самого CAS.
IDENTIFICATION_PURPOSES = frozenset({"product", "documents"})


def with_excluded_domains(query: str, purpose: str | None = None) -> str:
    """Дописывает к запросу вычёркивание заведомо бесполезных доменов.

    Запрос, который сам нацелен на конкретный домен, остаётся нетронутым:
    там оператор ``site:`` выражает намерение, и спорить с ним нельзя.

    Запрос на опознание вещества тоже остаётся нетронутым, и это выяснилось
    дорого. На боевом прогоне 322 по торговой марке «Plantacare 1200 UP»
    семь запросов из девяти вернули ноль, а прогон упал: открывать было
    нечего. Проверка на Serper показала причину — один и тот же запрос
    «Plantacare 1200 UP composition ingredients» без исключений даёт 8 из
    10 ссылок с названием марки, среди них страницу с INCI и страницу с
    CAS 110615-47-9, а с исключениями — ноль из десяти: ролики с рецептами,
    PubMed и заметку про майонез.

    Причина в том, что вычеркнутый ulprospector был первой ссылкой. Когда
    у запроса с ограничениями не остаётся совпадений, поисковик снимает
    ограничения и отдаёт что угодно. Каталог бесполезен как поставщик, но
    незаменим как справка о веществе, и путать эти две задачи нельзя.
    """
    text = (query or "").strip()
    if not text or re.search(r"(?<!-)\bsite:", text):
        return text
    if purpose in IDENTIFICATION_PURPOSES:
        return text
    return text + _EXCLUSION_SUFFIX
...
class MarketProfile:
    ...
    От страны к стране меняется язык слов роли: по числу результатов языки
    равны, но выдача разная — китайский запрос поднял Perstorp и
    Shin-Nakamura, которых английский не показал ни разу.

    **Ограничение по зоне** остаётся, хотя по первому взгляду выглядит
    провальным. Замер 3065 сохранённых запросов за 27 июля — 20 августа
    2026: запросы с любым `site:` довели дело до карточки в 19% случаев
    против 30% у запросов без него, а суженные до доменной зоны вернулись
    пустыми в 43% случаев. Убирать их, однако, нельзя.

    Решает не «дал ли запрос карточку», а «сколько карточек пропадёт, если
    убрать группу целиком»: у зоны это 23 карточки за месяц, 0,32 на
    запрос при базовой ставке 0,34. Зона возвращает мало, но возвращает то,
    чего не находит никто другой, — ровно потому, что сужает. Для сравнения,
    слова о выпуске дают 0,17 на запрос, а запросы о документах — 0,13.

    **Отраслевые реестры Индии** по той же метрике дали ноль: 15 запросов,
    61% пустых и ни одной карточки, которую не нашёл бы другой запрос того
    же прогона. Их из плана убрали. Домены остаются в
    `_INDIA_REGISTRY_DOMAINS`: страница реестра, попавшая в выдачу обычным
    запросом, по-прежнему распознаётся и ранжируется первой.
...
_MARKET_INDIA = MarketProfile(
    ...
    country_term="India",
    site_scope="site:.in",
    # Реестры убраны, зона и общий запрос остались.
    min_queries=3,
)
...
def build_search_queries(
    *,
    ...
    # плана: язык рынка открывает компании, которых нет в англоязычной
    # выдаче, и идёт первым, а сужение по зоне — последним. Реестров в
    # плане больше нет: см. MarketProfile, там замер, по которому их убрали.
    candidates: list[str | None] = []
    ...
    # Заход без номера идёт в обязательной голове плана, а не в хвосте.
    # Номер в точных кавычках сужает выдачу до страниц, где напечатан
    # именно он, и это отсекает рынок целиком, когда рынок пользуется
    # другим номером. У карбомера в заявке 9003-01-4 — полиакриловая
    # кислота, а косметический и фармацевтический грейд продаётся под
    # 9007-20-9 и марками 940, 980: запрос с номером не нашёл ни одного
    # из семи известных поставщиков, запрос по марке находит их сразу.
    # На эпоксидированном соевом масле тот же эффект: без номера находится
    # Hairma, крупнейший в мире, с номером — никто.
