...
| POST | `/rfq?start_search=true` | Создать закупочный запрос и поставить поиск заданного числа поставщиков по каждой выбранной стране в очередь |
...
| POST | `/supplier-search/jobs?rfq_id={id}` | Поставить полный цикл поиска и предварительной квалификации в очередь и сразу получить ID; worker не забирает задания до готовности локальной LLM |
| GET | `/search-runs?rfq_id={id}` | Очередь, история и текущие статусы поисков запроса |
| GET | `/search-runs/{id}` | Сводка, кандидаты и подробная трассировка этапов, промптов и источников |
...
первичных страниц на одной GPU может занимать больше пяти минут.
